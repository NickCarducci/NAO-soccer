...
SUBSCRIBER_NAME = "seek_to_walk_announcer"
POLL_INTERVAL = 0.2
...
def walk_toward_object_with_head_yaw(head_yaw):
    # Turn toward the object, then walk forward
    motion.moveTo(0, 0, head_yaw)  # Turn in place
    motion.moveTo(APPROACH_DISTANCE_METERS, 0.0, 0.0)  # Walk forward
        ...
        while not found:
            ...
            for pos in positions:
                ...
                for object_name in object_names:
                    # Only care if it matches a known prefix
                    for prefix in OBJECT_PREFIX_MESSAGES.keys():
                        if object_name.startswith(prefix):
                            if object_name != last_announced_object:
                                announce_object(object_name)
                                tts.say("Turning toward the object.")
                                walk_toward_object_with_head_yaw(pos)
                ...
                if found:
                    break
            if not found:
                tts.say("I couldn't find the object. Scanning again.")
                # Optionally, reset head to center before next scan
                motion.angleInterpolation("HeadYaw", [0.0], [1.0], True)
            time.sleep(POLL_INTERVAL)

        # Reset head to center after search
        motion.angleInterpolation("HeadYaw", [0.0], [1.0], True)
